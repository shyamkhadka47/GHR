...
const CustomerTestimonials = () => {
  ...
  const getalltestimonial = async () => {
    try {
      setloading(true);
      const res = await axiosInstance.get('/getalltestimonial');
  ...
  const handleDelete = async (so: any) => {
    try {
      const res = await axiosInstance.delete(`/deletetestimonial/${so}`);
      if (res.status == 200) {
        setShow(false);
        setId('');
            ...
            <div className="max-w-[1020px] mx-auto">
              ...
              <div className="relative flex flex-col w-full h-full overflow-hidden text-black-700 bg-white shadow-md rounded-lg bg-clip-border">
                    ...
                    {data &&
                      data?.map((el: any, ind: number) => (
                        ...
                        >
                          ...
                          <td className="">
                            <Link to={`/edit-testimonial/${el._id}`}>
                              {' '}
                ...
                <div className="mt-4 text-center ">
                  {data?.length < 1 && !loading && 'No Testimonial found'}
                </div>
              </div>
            </div>
          </div>
